...
def get_csv(path, plate_file):
    ...
    csv = [f for f in glob.iglob(os.path.join(path, "**/*.csv"),recursive=True) \
           if os.path.basename(f).startswith('ASO') or os.path.basename(f).startswith('R')]
    return csv
...
def assign_plates(df, file="ixCells_Round 1_2021-06-22_TN09_551ASOs_plate id adjusted.csv"):
    ...
    for i in range(len(df)): # try to find _P[0-9]+
        find_plate_no = re.search('P.*$', str(df.loc[i,'SampleName'])) # search for plate string
        if find_plate_no is not None:
            ...
            if find_plate_no.group(0) is not None:
                plate_no = find_plate_no.group(0)[1:]
                df.loc[i, 'Test plate #'] = plate_no
                continue
            else:
                find_plate_no = None
        # if samplename fails try ASO microsynth id
        else:
            find_plate_no = re.search('P.*$', str(df.loc[i,'ASO Microsynth ID']))  # search for plate number on microsynth id
        ...
        if find_plate_no is not None:
            ...
            if aso in plates['ASO Microsynth ID'].values:
                df.loc[i,'Test plate #'] = plates[plates['ASO Microsynth ID'] == aso]['Test plate #']

# old plates function
def assign_plates2(df, file="ixCells_Round 1_2021-06-22_TN09_551ASOs_plate id adjusted.csv"):
    plates = pd.read_csv(file, encoding='latin-1')
    plates.columns = plates.columns.str.strip()
    plates = plates[['Test plate #', 'ASO Microsynth ID']]
    df["Test plate #"] = np.nan
